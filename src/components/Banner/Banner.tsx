...
export const Banner = () => {
    ...
    const bannerElementsRef = useRef<HTMLDivElement | null>(null);
    const location = useLocation();

    useEffect(() => {
        if (bannerRef.current) {
            const elements = Array.from(bannerRef.current.querySelectorAll('h1, p, .bannerButtons a'));
    ...
    return (
        <div className='banner' ref={bannerRef}>
            <div className='banner' ref={bannerElementsRef}>
                <Badge color='info'>Nueva Colección</Badge>
                <h1 className='bannerTitle' ref={titleRef}>Descubre la moda de verano</h1>
